...
i=0
for j in {0..32}; do
  cp "../Shaders/Lever.gdshader" "../Shaders/${j}Lever.gdshader"
done
while IFS= read -r line; do
  ...
  if [ $((i%2)) = 0 ]
  then
    sed -i "s|^const vec4 PRIMARY_COLOR.*;$|const vec4 PRIMARY_COLOR=vec4(${s1},${s2},${s3},1);|" "../Shaders/$((i/2))Lever.gdshader"
  else
